...
Working Stochastic Scenarios Optimization
...
alpha_cvar = 0.1  # compute for worst 10% of the scenarios 
risk_weight = 1 # weight of the CVaR in teh objective function
...
#Flexibility (load UP / down) group availability
...
zeta = model.addVar(lb=-GRB.INFINITY, name="VaR_zeta") # VaR - threshold of profit for the worst alpha% scenarios, must be -infty for the lower limit
...
for scen in scenarios: 
    ...
    #Shortfall - How much each scenario falls below the VaR threshold , must be non-negative
    eta = model.addVars(
        no_of_scenarios, lb=0, 
        name="Shortfall_eta"
        ) 
...
#Obj Function+ Opt.+ eta constraint for selection  =======================================

penalty_flex = 1 # penalty factor, can be tailored to per hour based on the fex requirement

expected_profit_expr = 0
cvar_term_expr = 0

scenario_profit= [None] * no_of_scenarios

# 1. Calculate Profit for EACH Scenario 's'
for scen in scenarios:
    
    
    # Calculate revenue/cost specific to this scenario
    # Note: DA variables are constant across scenarios, ID/Battery variables vary
    scenario_profit[scen] = gp.quicksum(
        # DA Part (Same for all s)
        pi_sell_DA[t] * E_sell_DA[t] 
        - pi_buy_DA[t] * E_buy_DA[t] 
        # ID/Real-Time Part (Specific to s)
    ...
    # 2. Add to Expected Profit (Standard Average)
    expected_profit_expr += probability * scenario_profit[scen]
    
    # 3. Add CVaR Constraints (The Rockafellar Formulation)
    # Constraint: eta[s] >= zeta - Profit[s]
    # If Profit[s] is very low, eta[s] becomes positive to satisfy this.
    # If Profit[s] is high, (zeta - Profit) is negative, so eta[s] stays 0 (due to lb=0).
    model.addConstr(eta[scen] >= zeta - scenario_profit[scen], name=f"CVaR_Link_{scen}")

# 4. Formulate the CVaR Value
# CVaR = Threshold - (Sum of Shortfalls / alpha)
cvar_val = zeta - (1 / alpha_cvar) * gp.quicksum(probability * eta[s] for s in scenarios)

# 5. Set Final Objective: Weighted Average of Expected Profit and Risk
total_objective = (1 - risk_weight) * expected_profit_expr + (risk_weight) * cvar_val
...
if model.status == GRB.OPTIMAL:
    ...
    print(f"CVaR Threshold (Zeta): {zeta.X}")
    ...
    # Calculate actual numeric profit for each scenario for histogram
    final_scenario_profits = []

    for s in scenarios:
        # Calculate profit manually using .X values
        # Note: We must re-calculate because 'scenario_profit[s]' is a LinExpr, not a number
        p_s = sum(
            pi_sell_DA[t] * E_sell_DA[t].X 
            - pi_buy_DA[t] * E_buy_DA[t].X
            + pi_sell_ID[t] * E_sell_ID[s, t].X 
            - pi_buy_ID[t] * E_buy_ID[s, t].X
            - penalty_positive_imbalance[t] * E_imbalance_positive[s, t].X
            - penalty_negative_imbalance[t] * E_imbalance_negative[s, t].X
            - penalty_flex * (P_comp_down[s, t].X + P_comp_up[s, t].X)
            - pi_battery_degradation * (P_ch[s, t].X + P_dch[s, t].X)
            for t in range(T)
        )
        final_scenario_profits.append(p_s)
        
        # Store ID bids for spaghetti plot
        for t in time_steps:
            id_buy_vals[s, t] = E_buy_ID[s, t].X
            id_sell_vals[s, t] = E_sell_ID[s, t].X

    final_scenario_profits = np.array(final_scenario_profits)

    # --- 2. Plotting Bids (Spaghetti Plot) ---
    
    plt.figure(figsize=(14, 7))
    
    # Plot Intraday (ID) Scenarios
    for scen in scenarios:
        lbl_buy = 'ID Buy (Scenarios)' if scen == 0 else "_nolegend_"
        lbl_sell = 'ID Sell (Scenarios)' if scen == 0 else "_nolegend_"
        
        plt.step(time_steps, id_buy_vals[scen, :], where='mid', 
                 color='deepskyblue', alpha=0.15, linewidth=1, linestyle='--', label=lbl_buy)
        plt.step(time_steps, id_sell_vals[scen, :], where='mid', 
                 color='salmon', alpha=0.15, linewidth=1, linestyle='--', label=lbl_sell)

    # Plot Day-Ahead (DA) Bids
    ...
    plt.title(f'Market Bidding Strategy: DA vs ID Recourse ({no_of_scenarios} Scenarios)', fontsize=14)
    plt.ylabel('Energy Bid (MWh)', fontsize=12)
    plt.xlabel('Hour (t)', fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.legend(loc='upper right', frameon=True, shadow=True)
    
    # Highlight grid export restricted zones
    ...
    # --- 3. Plotting Profit Distribution (Histogram) ---
    
    plt.figure(figsize=(10, 6))
    
    # Histogram of profits
    plt.hist(final_scenario_profits, bins=20, color='skyblue', edgecolor='black', alpha=0.7, label='Scenario Profits')
    
    # Vertical lines for metrics
    plt.axvline(zeta.X, color='red', linestyle='dashed', linewidth=2, label=f'VaR (Zeta): {zeta.X:.0f}€')
    plt.axvline(np.mean(final_scenario_profits), color='green', linestyle='dashed', linewidth=2, label=f'Exp. Profit: {np.mean(final_scenario_profits):.0f}€')
    
    plt.title(f"Risk Profile: Profit Distribution (CVaR Weight: {risk_weight})")
    ...
    print(f"Model failed to solve. Status: {model.status}")
